...
import { generateFretboardMap, generateAllVoicingsForShape } from '@/lib/fretboardMap';
...
export function useDrawModeIndex({ allChordShapes, tuning, numFrets, rootNote }: DrawModeIndexParams) {
  ...
  const index = React.useMemo(() => {
    ...
    const isPositionBag = (bag: unknown): boolean => {
      ...
      return v && typeof v === 'object' && (
        'pattern' in v || 'altShapes' in v || ('rootString' in v && 'name' in v)
      );
    };

    function crawl(node: any, emit: (positions: any, trail: string[]) => void, trail: string[] = []) {
      if (!node || !node.options || typeof node.options !== 'object') return;
      if (isPositionBag(node.options)) emit(node.options, trail);
      for (const [label, child] of Object.entries(node.options) as [string, any][]) {
        if (child && child.levelName === 'Positions' && isPositionBag(child.options)) {
    ...
    for (const [difficulty, categories] of Object.entries(allChordShapes)) {
      for (const [category, node] of Object.entries(categories as Record<string, any>)) {
        crawl(node, (positions, trail) => {
          if (!positions || typeof positions !== 'object') return;
          for (const [posKey, base] of Object.entries(positions) as [string, any][]) {
            ...
            variants.forEach((formula, altIdx) => {
              if (!formula) return;
              const raw = generateAllVoicingsForShape(rootNote, formula, fretboardMap) || [];
