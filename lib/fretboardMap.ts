...
export function generateFretboardMap(tuning: string[], numFrets: number): string[][] {
  return tuning.map(openStringNote => {
    const startNoteIndex = NOTES.findIndex(pair => pair.includes(openStringNote));
    ...
    const newShape: NotePosition[] = pattern
      .map(note => {
        if (!note || note.fretOffset == null) {
          return { string: note?.string, fret: null, semitones: null, degree: null } as unknown as NotePosition;
        }
