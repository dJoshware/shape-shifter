...
export function toInitials(
  ...
  if (!str) return '';
  const parts = str.trim().split(/[^\p{L}\p{N}]+/u).filter(Boolean);
...
export function getFinalFormulasFromMatch(
  ...
  if (!matchInfo) {
    return { finalFormulas: null, availableAlts: [], posKey: '', altIdx: 0 };
  }
  ...
  const { difficulty, category, voicingType, stringSet, quality, posKey = '', altIdx = 0 } = matchInfo;

  let node: any = (shapes as any)?.[difficulty!]?.[category!];
  if (!node) return { finalFormulas: null, availableAlts: [], posKey: '', altIdx: 0 };
  ...
  const isPositionBag = (bag: unknown): boolean => {
    ...
    return v && typeof v === 'object' && ('pattern' in v || 'altShapes' in v || ('rootString' in v && 'name' in v));
  };

  const finalFormulas =
    node?.options && isPositionBag(node.options) ? (node.options as Record<string, unknown>) : null;

  let availableAlts: unknown[] = [];
  if (finalFormulas && posKey && finalFormulas[posKey]) {
    const base = finalFormulas[posKey] as any;
    availableAlts = [base, ...(Array.isArray(base.altShapes) ? base.altShapes : [])];
  }
