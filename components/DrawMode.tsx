...
import { useDrawModeIndex, keyFromSelection } from '@/lib/hooks/useDrawModeIndex';
import { allChordShapes, useCycleList, getFinalFormulasFromMatch } from '@/lib/API';
  ...
  if (vertical) {
    return (
      <div className="flex flex-col items-center gap-0.5">
        ...
        <button className={btnClass} onClick={onPrev}>−</button>
        <span className="text-[10px] font-bold text-center leading-tight min-w-[2.5rem] text-center">{display}</span>
        <button className={btnClass} onClick={onNext}>+</button>
      </div>
    );
  }
  return (
    <div className="flex items-center gap-2">
      <button className={btnClass} onClick={onPrev}>‹</button>
      <span className="text-sm font-bold min-w-[5rem] text-center">{display}</span>
      <button className={btnClass} onClick={onNext}>›</button>
    </div>
  );
}
...
export default function DrawMode() {
  ...
  const pickVoicingFor = React.useCallback((posKeyToUse: string, altIdxToUse: number): NotePosition[] | null => {
    if (!finalFormulas || !finalFormulas[posKeyToUse]) return null;
    const posData = finalFormulas[posKeyToUse] as any;
    const formula = (Number.isInteger(altIdxToUse) && posData.altShapes?.[altIdxToUse]) || posData;
    ...
    const best =
      voicings.find(v => Array.isArray(v) && v.length === targetSize && v.every((n: any) => drawStrings.has(n.string))) ||
      voicings[0];
    if (!best) return null;
  ...
  React.useEffect(() => {
    if (!matchInfo || !finalFormulas) { setSelectedPosition(''); setSelectedAltShape(0); return; }
  ...
  React.useEffect(() => {
    if (!matchInfo || !finalFormulas) { setSelectedPosition(''); setSelectedAltShape(0); lastFamilyRef.current = ''; return; }
    ...
    <div className="flex flex-col items-center w-full text-ink">
      ...
      <div className="sm:hidden w-full flex gap-1 mt-2 px-1">
        ...
        <div className="flex flex-col items-center gap-3 pt-14 w-14 shrink-0">
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            {rootMenuOpen && (
              <div className="fixed inset-0 z-50 flex items-start justify-start bg-black/30 pt-24 pl-3" onClick={() => setRootMenuOpen(false)}>
                <div className="bg-sand-2 rounded-xl shadow-xl p-2 max-h-56 overflow-y-auto grid grid-cols-3 gap-1" onClick={e => e.stopPropagation()}>
                  {NOTES.map(n => (
                    <button
                      key={n}
          ...
          {/* Clear */}
          <button onClick={clearAll} className="w-10 text-[10px] font-bold text-red-600 border border-red-600 rounded px-1 py-1 hover:bg-red-50 transition-colors">
            Clear
          </button>
          ...
          {positions.length > 0 && (
            <CycleControl
              label="Pos."
              display={((finalFormulas as any)?.[selectedPosition]?.name || selectedPosition) || '–'}
              onPrev={goPrevPos}
              onNext={goNextPos}
              vertical
            />
        ...
        <div className="flex flex-col items-center flex-1 min-w-0">
          ...
          <div className="bg-sand-4 border border-ink rounded-xl w-3/4 text-center py-2 my-3">
            <span className="text-lg font-semibold">{chordLabel}</span>
          </div>
          <FretboardVertical
      ...
      <div className="hidden sm:flex flex-col items-center w-full gap-4 py-4">
        ...
        {positions.length > 0 && (
          <div className="flex flex-wrap gap-2 justify-center">
            {positions.map(pos => (
              <button
                key={pos}
                onClick={() => { setSelectedPosition(pos); setBrowsedVoicing(pickVoicingFor(pos, selectedAltShape)); }}
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            <div className="flex gap-2">
              {availableAltsForUI.map((_, idx) => (
                <button
                  key={idx}
                  onClick={() => { setSelectedAltShape(idx); setBrowsedVoicing(pickVoicingFor(selectedPosition, idx)); }}
        ...
        <div className="bg-sand-4 rounded-xl px-12 py-4 text-center">
          <span className="text-3xl font-semibold">{chordLabel || root}</span>
        </div>
