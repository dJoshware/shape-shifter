...
import { deleteAccount, updateEmail, updatePassword, emailRegex, isValidPassword, toInitials } from '@/lib/API';
...
export default function Header({ difficulty, onDifficultyChange }: Props) {
  ...
  const [updateEmailAlert, setUpdateEmailAlert] = React.useState<{ msg: string; ok: boolean } | null>(null);
  ...
  const [updatePasswordAlert, setUpdatePasswordAlert] = React.useState<{ msg: string; ok: boolean } | null>(null);

  // Delete account
  const [deleteOpen, setDeleteOpen] = React.useState(false);
  const [deleteLoading, setDeleteLoading] = React.useState(false);
  const [deleteAlert, setDeleteAlert] = React.useState<{ msg: string; ok: boolean } | null>(null);
  ...
  React.useEffect(() => {
    ...
    async function wire(uid: string) {
      ...
      channel = supabase
        .channel('subs')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'subscriptions', filter: `user_id=eq.${uid}` }, payload => {
    ...
    (async () => {
      ...
      authSub = supabase.auth.onAuthStateChange((_evt, newSession) => {
        ...
        if (!newUid) { setHasPro(false); if (channel) supabase.removeChannel(channel); return; }
  ...
  const handleDifficultyClick = (level: Level) => {
    if (lockedLevels.includes(level)) { setPaywallOpen(true); return; }
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      if (!res.ok) { setPaywallAlert('Failed to create checkout session.'); return; }
  ...
  const handleUpdateEmail = async () => {
    ...
    try {
      ...
      if (email === user?.email) throw new Error('New email must be different from current email');
      await updateEmail(email);
      setUpdateEmailAlert({ msg: 'Email updated!', ok: true });
      await new Promise(r => setTimeout(r, 3000));
      setUpdateEmailAlert(null);
    } catch (err: any) {
      setUpdateEmailAlert({ msg: err.message.includes('Invalid email') ? 'Enter a valid email' : err.message.includes('different') ? 'Must be a new email address' : 'Could not update email.', ok: false });
    } finally {
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      ...
      if (!confirmPassword.trim()) throw new Error('Confirm your password');
      if (password !== confirmPassword) throw new Error('Passwords do not match');
      if (!isValidPassword(confirmPassword)) throw new Error('Password must be 8+ chars with uppercase, lowercase, number, and symbol.');
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      setDeleteAlert({ msg: 'Could not delete account. Try again.', ok: false });
    } finally {
  ...
  const levelBtnBase = 'relative px-3 sm:px-4 py-1.5 text-sm font-medium rounded border transition-all duration-150 select-none';
  const levelBtnActive = 'bg-sand-4 text-sand-1 border-ink font-semibold';
  const levelBtnInactive = 'bg-sand-1 text-ink border-ink/40 hover:border-ink';
  ...
  return (
    <>
      {/* ── AppBar ────────────────────────────────────────── */}
      <header className="relative flex items-center justify-between px-4 sm:px-8 pt-4 sm:pt-5 pb-2 sm:pb-3">
        {/* Left: Level selector */}
        <div>
          {/* Mobile: compact dropdown button */}
          <div className="sm:hidden flex flex-col items-start gap-0.5">
            <span className="text-[10px] font-bold text-ink/60 ml-0.5">Level</span>
            <button
            ...
            {mobileLevelOpen && (
              <div className="fixed inset-0 z-40 flex items-start justify-start bg-black/30 pt-14 pl-3" onClick={() => setMobileLevelOpen(false)}>
                <div className="bg-sand-2 rounded-xl shadow-xl overflow-hidden" onClick={e => e.stopPropagation()}>
                  {LEVELS.map(level => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative flex items-center gap-2 w-full px-5 py-3 text-sm font-medium border-b border-ink/10 last:border-0 transition-colors ${
                          difficulty === level ? 'bg-sand-4 text-sand-1 font-semibold' : 'text-ink hover:bg-sand-3'
                        } ${locked ? 'opacity-50' : ''}`}
                      >
                        {level}
                        {locked && <LockIcon />}
        ...
        <div className="flex items-center gap-2">
          {authIsLoading ? (
            ...
            <div className="relative">
              ...
              {menuOpen && (
                <div className="absolute right-0 top-12 z-30 bg-sand-2 border border-ink/20 rounded-xl shadow-xl overflow-hidden min-w-[150px]" onMouseLeave={() => setMenuOpen(false)}>
                  <button
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-full max-w-sm bg-sand-4 flex flex-col overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-ink/20">
              <h2 className="text-lg font-bold text-sand-1">Settings</h2>
              <button onClick={() => setDrawerOpen(false)} className="text-sand-1/70 hover:text-sand-1 transition-colors">
            ...
            <div className="flex-1 flex flex-col gap-6 px-5 py-5">
              {/* User ID */}
              <p className="text-xs text-sand-1/50 font-mono break-all">{user?.id}</p>
              ...
              <div className="flex flex-col gap-2">
                <h3 className="text-xs font-bold text-sand-1/70 uppercase tracking-wider">Email</h3>
                  ...
                  <div className="flex-1">
                    <FormFields
                      ...
                      onBlur={() => {
                        ...
                        if (!trimmed || !emailRegex.test(trimmed)) setEmail(user?.email ?? '');
                      }}
                      onChange={e => { setEmail(e.target.value); setUpdateEmailAlert(null); }}
                      type="email"
                      value={email}
                    />
                  </div>
                  <button
                    disabled={updateEmailLoading || email === user?.email}
                    onClick={handleUpdateEmail}
                    className={`px-3 py-1.5 rounded-full bg-ink text-sand-1 text-xs font-semibold transition-all shrink-0 ${
                      email === user?.email ? 'opacity-0 pointer-events-none' : 'opacity-100 hover:opacity-90'
                    } disabled:opacity-50`}
                  >
                    {updateEmailLoading ? '…' : 'Save'}
                  </button>
                </div>
                {updateEmailAlert && <p className={alertClass(updateEmailAlert.ok)}>{updateEmailAlert.msg}</p>}
              </div>

              {/* Change password */}
              <div className="flex flex-col gap-2">
                <h3 className="text-xs font-bold text-sand-1/70 uppercase tracking-wider">Password</h3>
                <FormFields
                  autoComplete="new-password"
                  endAdornment={
                    <button type="button" onClick={() => setShowPassword(s => !s)} className="text-sand-1/60 hover:text-sand-1 transition-colors">
                  ...
                  value={password}
                />
                <div className={`transition-all overflow-hidden ${password ? 'max-h-32 opacity-100' : 'max-h-0 opacity-0'}`}>
                ...
                {updatePasswordAlert && <p className={alertClass(updatePasswordAlert.ok)}>{updatePasswordAlert.msg}</p>}
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="w-full max-w-md bg-sand-4 rounded-2xl shadow-2xl overflow-hidden">
            <div className="px-6 py-5 border-b border-ink/20">
              <h2 className="text-xl font-bold text-sand-1 text-center">Unlock Pro Access</h2>
              <p className="text-sm text-sand-1/70 text-center mt-1">Get Advanced voicings and Draw Mode</p>
            </div>
            ...
            <div className="px-6 py-5 flex flex-col gap-4">
              ...
              <div className="flex rounded-xl overflow-hidden border border-ink/30">
                {(['monthly', 'yearly'] as const).map(p => (
                  ...
                  >
                    {p === 'monthly' ? 'Monthly' : 'Yearly (save 20%)'}
                  </button>
                ))}
              </div>

              {paywallAlert && (
                <p className="rounded-lg px-3 py-2 text-xs font-semibold bg-red-100 text-red-700 border border-red-300">{paywallAlert}</p>
              )}
            </div>
            ...
            <div className="flex gap-3 px-6 pb-5">
              ...
              >
                {!user ? 'Sign in first' : paywallLoading ? 'Loading…' : 'Subscribe'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/60 px-4">
          <div className="w-full max-w-sm bg-sand-4 rounded-2xl shadow-2xl overflow-hidden">
            <div className="px-6 py-5 border-b border-red-600/30">
              <h2 className="text-lg font-bold text-sand-1 text-center">Delete Account</h2>
              <p className="text-sm text-sand-1/70 text-center mt-1">This cannot be undone.</p>
            </div>
            <div className="px-6 py-4">
              {deleteAlert && <p className={alertClass(deleteAlert.ok)}>{deleteAlert.msg}</p>}
            </div>
            <div className="flex gap-3 px-6 pb-5">
              <button
                onClick={() => { setDeleteOpen(false); setDeleteAlert(null); }}
                className="flex-1 py-2.5 rounded-full bg-sand-2 text-ink text-sm font-semibold hover:bg-sand-3 transition-colors"
              >
                Cancel
              </button>
