...
export default function SubmitFeedback() {
  ...
  const [alert, setAlert] = React.useState<{ msg: string; ok: boolean } | null>(null);
  ...
  const removeFile = (name: string, lastModified: number) => {
    setFiles(prev => prev.filter(f => !(f.name === name && f.lastModified === lastModified)));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        setAlert({ msg: 'Feedback submitted! Check your spam folder for confirmation.', ok: true });
        setEmail('');
        setMessage('');
        setFiles([]);
        ...
        setAlert({ msg: 'Something went wrong. Please try again.', ok: false });
      }
    } catch {
      setAlert({ msg: 'An error occurred. Please try again.', ok: false });
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md bg-sand-4 rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <div className="border-b border-ink/30 px-6 py-4 shrink-0">
              <h2 className="text-xl font-bold text-sand-1 text-center">Submit feedback</h2>
            </div>
            ...
            <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-4 overflow-y-auto">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-sand-1/80">Your email</label>
                <input
              ...
              {/* Message */}
              <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-sand-1/80">Description</label>
                <textarea
                  ...
                  onChange={e => { setMessage(e.target.value); setAlert(null); }}
              ...
              <div className="flex flex-col gap-2">
                <span className="text-xs font-semibold text-sand-1/80">Screenshot (optional)</span>
                ...
                {files.length > 0 && (
                  <ul className="flex flex-col gap-1">
                    {files.map(f => (
                      <li key={`${f.name}-${f.lastModified}`} className="flex items-center gap-2 text-xs text-ink/80">
                        <span className="truncate">{f.name}</span>
                        <button type="button" onClick={() => removeFile(f.name, f.lastModified)} className="ml-auto text-ink/50 hover:text-red-600 transition-colors">✕</button>
                      </li>
                    ))}
                  </ul>
                )}
              ...
              {alert && (
                <div className={`rounded-lg px-3 py-2 text-sm font-semibold ${alert.ok ? 'bg-green/20 text-green border border-green/40' : 'bg-red-100 text-red-700 border border-red-300'}`}>
                  {alert.msg}
                </div>
              )}
            </form>
            ...
            <div className="flex justify-end gap-3 px-6 pb-5 shrink-0">
              <button
                type="button"
                onClick={() => { setOpen(false); setAlert(null); }}
