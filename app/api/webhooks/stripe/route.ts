...
function toRow(sub: Stripe.Subscription, userId: string | null) {
  ...
  return {
    ...
    current_period_end: firstItem
      ? new Date((firstItem as any).current_period_end * 1000).toISOString()
      : null,
  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (!row.user_id) { console.warn('No user_id → skipping'); break; }
      ...
      case 'customer.subscription.updated': {
        ...
        if (!userId && sub.customer) {
          const { data: existing, error } = await supabaseAdmin
            ...
            .maybeSingle();
          if (error) console.error('Lookup by customer error:', error);
