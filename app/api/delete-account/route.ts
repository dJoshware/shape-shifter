...
export async function DELETE(req: Request) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to delete account', details: error.message }, { status: 400 });
    }

    return NextResponse.json({ success: true, message: 'Account successfully deleted' });
  } catch (err: any) {
    console.error('Unexpected deletion error:', err);
    return NextResponse.json({ error: 'Unexpected error during account deletion', details: err.message }, { status: 500 });
  }
}
