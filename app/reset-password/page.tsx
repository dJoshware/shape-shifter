...
export default function PasswordResetPage() {
  ...
  const validateForm = () => {
    setFormError('');
    setAlertMessage('');
    if (!password.trim()) { setFormError('Please enter a password.'); return false; }
    if (!confirmPassword.trim()) { setFormError('Please confirm your password.'); return false; }
  ...
  React.useEffect(() => {
    if (confirmPassword.length > 0) {
      setFormError(confirmPassword === password ? '' : 'Passwords must match.');
    } else {
      setFormError('');
    }
    ...
    <div className="flex-1 grid place-items-center px-4 py-8">
      <div className="w-full max-w-sm">
        <div className="bg-sand-4 rounded-2xl shadow-xl p-6 sm:p-8 flex flex-col gap-5 text-center">
          <h3 className="text-2xl font-bold text-sand-1">Reset your password</h3>
